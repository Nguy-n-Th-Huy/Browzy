...
**Xem mức sử dụng hiện tại.** Settings hiển thị mức sử dụng và giới hạn hiện tại của tài khoản đã đăng nhập: mỗi cửa sổ giới hạn mà tài khoản báo về (một gói có thể có một hoặc hai cửa sổ) hiện phần trăm đã dùng và đồng hồ đếm ngược tới mốc đặt lại, kèm nút **Làm mới** để đọc lại khi bạn muốn. Giá trị đọc từ đúng tài khoản đã đăng nhập mà profile đang chạy — đọc khi trang mở một profile đã đăng nhập và khi bạn bấm Làm mới, không bao giờ đọc theo hẹn giờ — và việc đọc này **không** tính vào giới hạn sử dụng mô hình, khác với **Kiểm tra kết nối** ở trên. Đây vẫn là backend không chính thức, không có tài liệu như đường chạy; nếu endpoint đó thay đổi hoặc đóng, lỗi chỉ nằm trong khối mức sử dụng, phần còn lại của Settings vẫn hoạt động bình thường.
