...
await test("default mode (host/mcp-server.js): starts, lists the full registry, and calls a tool — env limited to the MCP SDK's own minimal allowlist (no ANTHROPIC_*/CLAUDE_* anywhere)", async () => {
  ...
  try {
    ...
    assert(tools.length === 33, `expected 33 registry tools (26 preserved + 8 post-baseline - 1 removed), got ${tools.length}`);
...
await test("hybrid mode (server-hybrid.js): starts, exposes all 33 upstream tools + execute_code + recording_ack, and a direct passthrough call reaches the extension — no ANTHROPIC_*/CLAUDE_* env, no companion", async () => {
  const pipe = pipeFor(++seq);
  const ext = fakeExtension(pipe);
  ...
  try {
    ...
    assert(upstreamCount === 33, `expected 33 upstream passthrough tools (26 preserved + 8 post-baseline - 1 removed), got ${upstreamCount}: ${names.join(", ")}`);
