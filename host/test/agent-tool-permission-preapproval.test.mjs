...
//   1. All 33 registry-backed browser tools, namespaced under this run's mcp
//      server name, plus "Skill", are present in the SDK's `tools`
//      (availability) allowlist.
//   2. Every ALWAYS-AUTOMATIC registry-backed browser tool (32 minus the
...
await test("all 33 registry-backed browser tools, namespaced under this run's mcp server, plus Skill, are preapproved — nothing silently missing", async () => {
  assert(
    TOOLS.length === 33,
    `expected the known 33-entry registry baseline (26 preserved + 8 post-baseline - 1 removed; see test/registry-baseline.test.mjs), got ${TOOLS.length} — if the registry grew or shrank on purpose, this assertion must be updated deliberately, not silently`
  );
...
await test("all 29 always-automatic browser tools are auto-approved via allowedTools; computer, javascript_tool, browser_batch & webmcp_call_tool stay available through tools only (task 9.1, extended)", async () => {
  ...
  // The remaining 29 browser tools stay in `allowedTools` unchanged — zero
  // added latency,
  ...
  // `allowedTools`) is still caught here for the 29 always-automatic tools.
  assert(
    TOOLS.length === 33,
    `expected the known 33-entry registry baseline (26 preserved + 8 post-baseline - 1 removed; see test/registry-baseline.test.mjs), got ${TOOLS.length} — if the registry grew or shrank on purpose, this assertion must be updated deliberately, not silently`
  );
  ...
  assert(alwaysAutomatic.length === 29, `internal sanity: 29 always-automatic tools expected, got ${alwaysAutomatic.length}`);
  ...
  // WebSearch and Task are auto-approved alongside the 29 always-automatic
  ...
  assert(
    ...
    `allowedTools must contain exactly the 29 always-automatic browser tools plus WebSearch and Task (${alwaysAutomatic.length + 2}), no more and no fewer — got ${options.allowedTools.length}: ${JSON.stringify(options.allowedTools)}`
  );
