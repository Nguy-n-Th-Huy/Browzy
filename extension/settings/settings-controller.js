...
// left UNCONTROLLED by controller state. `save(secretInput, jevToolsSecrets)`
...
// field would. That is both keys of the page — the primary Anthropic/ChatGPT
// one (`secretInput`) and the Jev browser-tools transport key
// (`jevToolsSecrets`) — on one rule, with the same clearing behavior in the
// DOM layer. The only place a raw key value is EVER held by this class is
...
// holds either the primary credential's string or the Jev transport key's
// `{ typesafeApiKey }` object, and exists only to let an explicit, user-
// confirmed memory-only retry proceed after a SECURE_STORAGE_UNAVAILABLE
// failure without forcing the user to retype the key they just submitted; it
// is cleared (`= null`) after every save attempt's outcome, on
// `init()`/`switchProfile()`, and on `removeCredential()`/`removeTypesafeKey()`.
...
function emptyState(profileId) {
  return {
    ...
    // the has-key boolean and the Jev source choice for the transport
    // `extract_page`/`browser_subgoal` use. The key VALUE is never readable —
    // it lives in the OS credential store and comes back from the companion
    // as a boolean only (see settings-client.js's wire contract), which is
    // why the key input stays uncontrolled like the primary one (file
    // header).
    ...
    // _applyProfile; setJevToolsTransportSource() refuses anything else.
    typesafeSource: "typesafe",
    // The Jev-tools screenshot toggle (jev-subgoal-screenshots-default-off
    // task 3.2): whether a `browser_subgoal` sub-run started from THIS
    // anthropic/chatgpt profile captures a screenshot for its optional
    // planning/content model. A non-secret profile field persisted by
    // saveJevTools() through set_typesafe_config, DISABLED by default — an
    // absent field (a profile stored before the toggle existed, or a
    // companion that predates it) loads as disabled, which is the documented
    // default (host/agent/settings/profile-schema.js's
    // resolveJevToolsSendScreenshots()); only an explicit `true` enables it.
    jevToolsSendScreenshots: false,
    ...
    saving: false,
    // The Jev browser-tools section's own Save button
    // (jev-tools-settings-on-llm-profiles), for an anthropic/chatgpt
    // profile: a separate in-flight flag from `saving` above so the page's
    // primary Save button and this section's own Save button never disable
    // each other.
    savingJevTools: false,
    testing: false,
    // The Jev-tools connection test (jev-tools-connection-test-and-preference
    // tasks.md 2.2): a separate in-flight flag from `testing` above, since the
    // primary provider test and this section's own test run independently and
    // must never disable each other.
    testingJevTools: false,
    discovering: false,
    ...
    // The Jev-tools connection test's own last result, rendered inside
    // #jevtools-fields ONLY — deliberately never merged into `connectionStatus`
    // above, which is the PRIMARY provider's own test and must stay untouched
    // by this one (spec `agent-settings`: "distinct from the primary
    // provider's connection test").
    // { status: "testing"|"pass"|"fail"|"not_configured", tools: {extract_page, browser_subgoal}, capabilities, errors, timestamp } | null
    jevToolsTest: null,
...
// --- Jev transport source facts and copy -----------------------------------
//
// The Jev browser tools' own transport source (jev-tools-settings-on-llm-
// profiles): which service `extract_page`/`browser_subgoal` call for element
// selection. The documented endpoints below are hand-synced from
...
// Jev-tools transport section names the selected source on every source-
// dependent surface — the endpoint hint, the key field, and the key's remove
// action — all of which read this one table.
//
 ...
 * one anyway (_applyProfile coerces, setJevToolsTransportSource refuses). */
...
export class SettingsController {
  ...
  _applyProfile(profile) {
    ...
    // Jev browser tools' own transport (jev-tools-settings-on-llm-profiles):
    // the has-key boolean and the Jev source, taken verbatim from the
    // companion's secret-free profile. Absent (a profile saved before Jev
    // browser tools existed) means false/default, never a stale value
    // carried over from a previously displayed profile — this runs on a
    // fresh state after every init()/switchProfile(), but also after
    // setProviderType()/save() replies.
    ...
    // The Jev-tools screenshot toggle (jev-subgoal-screenshots-default-off
    // task 3.2). The companion resolves the default and sends a boolean, but
    // an ABSENT field — a profile stored before the toggle existed, or a
    // companion older than the setting — loads as DISABLED, the documented
    // default. Only an explicit `true` enables it.
    s.jevToolsSendScreenshots = profile.jevToolsSendScreenshots === undefined ? false : Boolean(profile.jevToolsSendScreenshots);
    ...
    if (s.providerType === "chatgpt" && s.chatgptSessionState === "session_expired" && !s.banner) {
  ...
  async setProviderType(providerType) {
    ...
    try {
      ...
      // Same reasoning for the Jev-tools test: a switch means whatever it
      // last reported no longer describes this profile.
      this.state.jevToolsTest = null;
  ...
  _usageReadable() {
    const s = this.state;
    return s.providerType === "chatgpt" && s.chatgptSessionState === "signed_in";
  }
  ...
  // --- Jev browser-tools transport (jev-tools-settings-on-llm-profiles) ---

  /** The Jev browser-tools section's own transport-source select
   * (jev-tools-settings-on-llm-profiles), for an anthropic/chatgpt profile.
   * This never touches `state.baseUrlDraft` — the profile's PRIMARY
   * Anthropic/ChatGPT endpoint — which this section must never alter (specs/
   * agent-settings "Primary provider configuration is untouched"). The Jev
   * transport endpoint is never stored: host/agent/settings/profile.js's
   * resolveJevBrowserSubgoalConfig always derives it from the source via
   * `typesafeDefaultForSource`. So this setter touches ONLY
   * `state.typesafeSource` and nothing else. */
  setJevToolsTransportSource(source) {
  ...
  /** The Jev-tools screenshot toggle (jev-subgoal-screenshots-default-off
   * task 3.2) — its own local state field. saveJevTools() is the only
   * writer, and it persists the value through the `set_typesafe_config` op
   * (the field is profile state, not a per-conversation switch). No
   * validation can fail here — the DOM layer feeds it a checkbox's boolean,
   * and every value is coerced to the profile's own boolean type so nothing
   * else can ride into the field. */
  setJevToolsSendScreenshots(enabled) {
    this.state.jevToolsSendScreenshots = Boolean(enabled);
    this._notify();
    return { ok: true };
  }
  ...
  /** The Jev browser-tools section's own transport-key save — the piece
   * save() and saveJevTools() both need, extracted so there is exactly one
   * place that collects the typed key, sends it through
   * `set_typesafe_credentials`, and applies the reply/error. Sent only when
   * the caller actually typed a key: an OMITTED key keeps the stored value,
   * while an explicit "" REMOVES it (see settings-client.js's wire contract)
   * — an untouched input is therefore left out of the payload entirely,
   * never sent as "". The reply is a boolean plus the storage backend;
   * nothing key-shaped comes back. This key describes the Jev transport
   * only — it never touches `hasCredential`/`memoryOnlyCredential`/
   * `secretBackend`, which describe the profile's PRIMARY Anthropic/ChatGPT
   * credential, a separate, already-saved secret this call must never
   * overwrite.
   *
   * @param {{ typesafeApiKey?: string }} typesafeSecrets the raw key value,
   *   exactly as the caller received it from its own DOM input — never
   *   assigned to `this.state`, only ever sent. */
  async #saveJevToolsCredentials(typesafeSecrets) {
    const typedTypesafeKey = typeof typesafeSecrets.typesafeApiKey === "string" ? typesafeSecrets.typesafeApiKey.trim() : "";
    if (!typedTypesafeKey) return;
    const keys = { typesafeApiKey: typedTypesafeKey };
    ...
    try {
      ...
      // Any key change invalidates a previously recorded capability result
      // host-side; the page must not keep showing the old pass.
      this.state.connectionStatus = null;
      this.#pendingSecretForRetry = null;
      ...
      if (err.code === "SECURE_STORAGE_UNAVAILABLE") {
        // Same explicit, labeled memory-only offer as the API-key path in
        // save() (specs/agent-settings "Secret isolation").
   ...
   * @param {{ typesafeApiKey?: string }} [jevToolsSecrets]
   *   the Jev browser-tools section's OWN key value (jev-tools-reuse-primary-
   *   provider — the Jev transport key), read LIVE from its own DOM input at
   *   the same moment and handled exactly like `secretInput` above: this
   *   page's one bottom Save button is the fix for that section previously
   *   having no key-saving path other than its own, easy-to-miss
   *   #btn-save-jevtools.
   *   Defaulted rather than positional on purpose: neither of these two
   *   arguments is another handle on the profile — the no-conversation-leak
   *   assertion "save() takes at most one parameter"
   *   (test/settings-ui-no-conversation-leak.test.mjs) stays literally true
   *   (`Function#length` is 1 with a default) and there is still no way to
   *   pass a conversation through here. */
  async save(secretInput, jevToolsSecrets = {}) {
    ...
    // Jev-tools ops need it (a companion that predates them answers with its
    // own unknown-op PROTOCOL_ERROR, which reads as "update the companion" —
    // see errors-ui.js's TYPESAFE_OPS); it is recorded rather than inferred,
    // so it stays right whichever half fails.
    let inFlightOp = null;
    try {
      ...
      // The Jev browser-tools section's own transport key
      // (jev-tools-settings-on-llm-profiles), sent through the shared
      // #saveJevToolsCredentials() helper (see its doc comment) — this page's
      // one bottom Save button is the fix for that section previously having
      // no key-saving path other than its own #btn-save-jevtools.
      inFlightOp = "set_typesafe_credentials";
      await this.#saveJevToolsCredentials(jevToolsSecrets);
      ...
      // the two Jev-tools ops change the copy (see errors-ui.js's
  ...
  /** Save the Jev browser-tools configuration for an anthropic/chatgpt
   * profile (jev-tools-reuse-primary-provider). Distinct from save() above:
   * it never sends the profile's primary base URL, model list, or
   * Anthropic/ChatGPT credential — and it collects NO text-model fields at
   * all, because the tools' text/decision model is the profile's OWN
   * primary provider and current model, resolved host-side from the run
   * snapshot (host/agent/settings/profile.js's
   * resolveJevExtractPageConfig/resolveJevBrowserSubgoalConfig). This method
   * sends only `typesafeSource` (the Jev transport source) and
   * `jevToolsSendScreenshots`, using the SAME state/setter
   * (`typesafeSource`/`setJevToolsTransportSource`) — the host stores it as a
   * flat profile field. Saving the transport key (below) is the ONLY switch:
   * it enables `extract_page` and `browser_subgoal` together.
   *
   * The endpoint is never sent: `resolveJevBrowserSubgoalConfig` always
   * derives it from the transport source (`typesafeDefaultForSource`), never
   * from `profile.baseUrl` — which on this profile type holds the Anthropic
   * endpoint. Sending a `baseUrl` here would silently overwrite it.
   *
   * `state.hasCredential`/`memoryOnlyCredential`/`secretBackend` are
   * deliberately left untouched by the credentials half below: they describe
   * the primary Anthropic/ChatGPT credential, not the Jev transport key, and
   * must never be overwritten by a Jev-tools save.
   *
   * @param {{ typesafeApiKey?: string }} [typesafeSecrets]
   */
  async saveJevTools(typesafeSecrets = {}) {
    this.state.savingJevTools = true;
    this.state.banner = null;
    this._notify();

    const pendingTransportSource = this.state.typesafeSource;
    let inFlightOp = null;
    try {
      inFlightOp = "set_typesafe_config";
      const config = await this.client.setTypesafeConfig(this.state.profileId, {
        typesafeSource: pendingTransportSource,
        // The Jev-tools screenshot toggle
        // (jev-subgoal-screenshots-default-off task 3.2), sent explicitly on
        // every save so unchecking it in the DOM actually reaches the wire
        // (an omitted value would keep whatever is stored, exactly like
        // every other field here).
        jevToolsSendScreenshots: this.state.jevToolsSendScreenshots
        // No `baseUrl`, no text-model fields: see this method's doc comment
        // above.
      });
      this._applyProfile(config);
      // The config just changed host-side, so a previously recorded Jev-tools
      // test result no longer describes this profile — same reasoning as
      // `connectionStatus` below for the primary test.
      this.state.jevToolsTest = null;

      // The write-only transport key, sent through the SAME shared
      // #saveJevToolsCredentials() helper save() uses. `inFlightOp` is still
      // recorded first so an error escaping the helper's own try/catch (there
      // should not be one) still gets the right op label from the outer
      // catch below.
      inFlightOp = "set_typesafe_credentials";
      await this.#saveJevToolsCredentials(typesafeSecrets);

      this.state.savingJevTools = false;
      if (!this.state.banner) {
        this.state.banner = { kind: "success", title: "Đã lưu", message: "Đã lưu cấu hình Jev browser tools.", action: "" };
      }
      this._notify();
      return { ok: true };
    } catch (err) {
   ...
   * `memoryOnly: true`. Two shapes reach the private field: a bare string
   * (the primary Anthropic/ChatGPT API key) and `{ typesafeApiKey }` (the
   * Jev browser-tools transport key). Which one is pending is decided by its
   * own type, not by `state.providerType`. */
  async confirmMemoryOnlyCredential() {
    ...
    try {
      if (isTypesafePair) {
        ...
        this.state.hasTypesafeKey = Boolean(result.hasTypesafeKey);
        // This is the Jev browser-tools transport key: `hasCredential`/
        // `memoryOnlyCredential`/`secretBackend` describe the PRIMARY
        // Anthropic/ChatGPT credential instead, which this retry never
        // touches — overwriting them here would misreport that credential as
        // gone/memory-only.
        this.state.connectionStatus = null;
        // This retry's own last Jev-tools test is stale too.
        this.state.jevToolsTest = null;
  ...
  /** Remove the Jev browser-tools transport key — the "remove" action the
   * write-only key field carries (jev-tools-settings-on-llm-profiles). An
   * explicit empty string is the wire's removal signal (see
   * settings-client.js's wire contract). The reply's boolean then drives the
   * page, so a removal that the companion refused cannot leave the UI
   * claiming the key is gone. Never touches `hasCredential`/
   * `memoryOnlyCredential`/`secretBackend`, which describe the PRIMARY
   * Anthropic/ChatGPT credential — clearing them here would misreport that
   * credential as gone, even with it still saved. */
  async removeTypesafeKey() {
    this.state.removingCredential = true;
    this._notify();
    try {
      const result = await this.client.setTypesafeCredentials(this.state.profileId, { typesafeApiKey: "" });
      this.state.hasTypesafeKey = Boolean(result.hasTypesafeKey);
      this.state.connectionStatus = null; // credential changed -> prior results invalidated (host-side truth)
      // The Jev-tools section's own "remove key" — its last test result is
      // stale too.
      this.state.jevToolsTest = null;
      this.#pendingSecretForRetry = null;
      this.state.pendingMemoryOnlyOffer = false;
      ...
      this.state.banner = {
        kind: "info",
        title: "Đã xóa API key TypeSafe",
        message: "Cần nhập lại API key trước khi kiểm tra kết nối.",
        action: ""
      };
      this.state.removingCredential = false;
      this._notify();
      return { ok: true, hasTypesafeKey: false };
    } catch (err) {
      this.state.removingCredential = false;
      this.state.banner = { kind: "error", code: err.code, ...describeErrorCode(err.code, { op: "set_typesafe_credentials" }) };
      this._notify();
  ...
  /**
   * The Jev-tools connection test (jev-tools-connection-test-and-preference
   * tasks.md 2.2): a `test_capability` call with `target: "jev-tools"`,
   * separate from `testConnection()` above — it validates the Jev
   * browser-tools config on this `anthropic`/`chatgpt` profile, not the
   * profile's own primary provider, and never touches `connectionStatus` or
   * the page's top `banner` (those stay the primary test's own). The result
   * is rendered entirely inside `#jevtools-fields` from `state.jevToolsTest`.
   *
   * No credential/model gate is checked first (unlike `testConnection()`):
   * the host itself reports `status: "not_configured"` when the Jev fields
   * are empty, rather than a false pass or a thrown error, so the button has
   * nothing else to gate on.
   */
  async testJevToolsConnection() {
    this.state.testingJevTools = true;
    this.state.jevToolsTest = { status: "testing" };
    this._notify();
    try {
      const result = await this.client.testJevToolsCapability(this.state.profileId);
      this.state.jevToolsTest = result;
      this.state.testingJevTools = false;
      this._notify();
      return { ok: result.status !== "fail" };
    } catch (err) {
      this.state.jevToolsTest = {
        status: "fail",
        tools: { extract_page: false, browser_subgoal: false },
        capabilities: {},
        errors: { connection: { code: err.code, message: err.message } }
      };
      this.state.testingJevTools = false;
      this._notify();
      return { ok: false, error: err.message, code: err.code };
  ...
  async importProfile(imported) {
    ...
    // The Jev-tools screenshot toggle rides the same secret-free document
    // (jev-subgoal-screenshots-default-off task 3.2); staged like the field
    // above, so the next Save (saveJevTools()) — and only it — writes it back.
    if (typeof imported.jevToolsSendScreenshots === "boolean") {
      this.state.jevToolsSendScreenshots = imported.jevToolsSendScreenshots;
    }
    this.state.banner = {
      kind: "info",
