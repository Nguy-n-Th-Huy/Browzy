...
console.log("== provider type: switching to chatgpt calls set_provider_type and keeps the model list ==");
{
  ...
  // The removed standalone `typesafe` provider type is no longer a
  // selectable, storable provider type — the controller rejects it locally
  // exactly like any other unrecognized value, never sending it to the
  // companion.
  const typesafeAttempt = await c.setProviderType("typesafe");
  ok(typesafeAttempt.ok === false && companion.calls.length === before, "\"typesafe\" is rejected locally, never sent to the companion");
  ok(c.getState().providerType === "chatgpt", "the rejected typesafe switch leaves the current provider type intact");
}
