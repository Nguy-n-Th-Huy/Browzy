...
Both share the same 33-tool browser registry (25 preserved + 8 post-baseline additions, with 1 removal recorded), the same stable extension
...
Components:
1. **Extension** — Manifest V3 with CDP-based browser automation (33 registry tools: 25 preserved + 8 post-baseline additions, with 1 removal recorded), plus a side panel and settings UI
...
`zod@^4` peer, while this project's existing 33-tool registry depends on
...
Either type can additionally turn on the **Jev browser tools** (`extract_page`,
`browser_subgoal`) — see [Jev browser tools](#jev-browser-tools) below.

> A profile whose stored `providerType` was the removed standalone `typesafe`
> ("Jev — ultrafast") type is migrated automatically to `anthropic` the next
> time it loads: its Base URL resets to the default Anthropic endpoint, any
> Jev-seeded model list is cleared (a custom model list is kept), and its
> saved Jev transport key is kept as-is, so the Jev browser tools keep working
> once a primary Anthropic/ChatGPT credential is entered.
...
`extract_page` and `browser_subgoal` are two optional tools any **API tương
thích Anthropic** or **Tài khoản ChatGPT** profile can turn on. Neither needs
its own model configuration: both reuse the profile's own primary provider and
currently selected model — the same credential from [Configure your
provider](#configure-your-provider) above — as their text/decision model. The
only thing they need beyond that is a **Jev transport key**, which is what
lets `browser_subgoal` reach Jev's structured-choice decisions.

1. **Turn them on.** In **Settings**, the **Jev browser tools** section is
   shown for every profile. Choose the **Nguồn vận chuyển Jev** ("Jev transport
   source"): **TypeSafe API**, **Vercel AI Gateway**, or **OpenRouter
   (alpha)**. Enter and save the transport key for that source. Saving that
   key is the only switch: once it is saved, both tools appear together; while
   it is missing or cleared, neither appears, and an incomplete configuration
   never errors — the tools simply don't show up.
2. **`extract_page`** offloads one bounded, read-only field extraction from the
   bound tab: the caller names the fields and types it wants, the tool
   observes the page through the same `page_snapshot` path the runtime's own
   observation uses, and the profile's own text model answers with exactly
   those keys — every field nullable when evidence is absent or ambiguous. It
   never mutates the page and needs no approval card.
3. **`browser_subgoal`** delegates one bounded natural-language interaction
   (e.g. "fill the search form with origin/destination/date and submit") to a
   short Jev sub-run on the caller's own bound tab — the same lease, never a
   new tab. Each cycle asks Jev three questions from one structured-choice
   request: which complete action to take, whether the goal may be complete,
   and whether progress is stuck. The sub-run always returns an **unverified
   checkpoint** — the observed page, a bounded summary of the actions it took,
   and why it stopped — and never claims the goal was achieved; the calling
   model does the final verification.
4. **Ảnh chụp màn hình (tuỳ chọn)** — off by default. When on, each
   `browser_subgoal` cycle also sends a screenshot of the controlled tab to
   the profile's primary provider, for that provider's own optional
   planning/content step. Jev's own action selection is unaffected either way:
   it always decides from the structured `page_snapshot`, never from an image.

Every dispatch behind these tools passes exactly the same host-side
authorization any other tool call passes: run-state and lease checks, tab
scope, the protected-action backstop, and the same approval card when a click
could submit, send, pay, or confirm.

**Usage and billing follow those services separately**: the `browser_subgoal`
structured-choice requests are billed by whichever Jev transport source you
chose (TypeSafe, Vercel AI Gateway, or OpenRouter); the profile's primary
provider bills whatever calls the tools make to it (the `extract_page`
extraction call, and `browser_subgoal`'s optional planning screenshot).
...
(`openspec/changes/add-workflow-materialization-and-heal`; see below), and a
`browser_subgoal` sub-run's decision steps and terminal outcome appearing
inline in the turn that called it — one row per step naming the action Jev
...
and a terminal line reporting the unverified checkpoint the tool returned to
the calling model. Historical records retain their original attribution
(`openspec/changes/add-typesafe-jev-provider`, extended by
`openspec/changes/add-jev-run-context` and `openspec/changes/add-jev-run-screenshots`
— the latter adds no new rows: a page capture surfaces in the action timeline
exactly like any other capture; see [Jev browser tools](#jev-browser-tools)
above).
...
The **hybrid** server exposes everything: all 33 tools directly (25 preserved + 8 post-baseline additions), `execute_code`
...
**Default** — the 33 tools, nothing else:
```bash
claude mcp add browzy-in-chrome -- node /absolute/path/to/host/mcp-server.js
```

**Code mode** — three tools: `execute_code`, `screenshot`, `zoom`. The model writes JS that calls `chrome.*` (the typed API for all 33 tools) in a sandboxed Cloudflare Worker, collapsing multi-step flows into one round trip:
```bash
...
The core browser registry (`host/tool-definitions.js`) has **33** entries,
...
`debug_timings`) plus the **8** post-baseline additions,
...
per-step observation, and equally usable by a model-driven run), and
`page_monitor` (a generic page monitor backed by observed JSON network
responses) — each
tracked as an
...
against the same 33-tool registry; the side panel does not yet expose
...
| `page_monitor` | Save and compare a page baseline from an observed JSON network response | |
